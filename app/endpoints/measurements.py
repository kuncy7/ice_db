...
def create_measurement_for_rink(
    ice_rink_id: UUID,
    ...
    """
    Dodaje nowy pomiar dla konkretnego lodowiska.
    """
    ...
    # Można dodać walidację, czy użytkownik ma uprawnienia do tego lodowiska
...
@router.get("/{ice_rink_id}/measurements", response_model=List[MeasurementSchema])
def read_measurements(
    ...
    """
    Pobiera listę pomiarów dla lodowiska, z opcjonalnym filtrowaniem po dacie.
    """
...
def read_latest_measurement(
    ...
    """
    Pobiera najnowszy pomiar dla danego lodowiska.
    """
...
def export_measurements(
    ...
    """
    Eksportuje dane pomiarowe z zadanego okresu do pliku CSV lub JSON.
    """
    query = db.query(Measurement).filter(Measurement.ice_rink_id == ice_rink_id)
    query = query.filter(Measurement.timestamp >= start_date)
    query = query.filter(Measurement.timestamp <= end_date)
    ...
    if format == "json":
        # Prosty eksport do JSON - zwracamy listę jako plik
        output = [MeasurementSchema.from_orm(m).dict() for m in measurements]
        return output
    ...
    if format == "csv":
        ...
        # Nagłówki
        headers = MeasurementSchema.__fields__.keys()
